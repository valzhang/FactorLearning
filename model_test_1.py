...
def GeneratePoolData(csv_path):
    ...
    return pool_data
pool = GeneratePoolData(csv_path=dl_path)
...
uni = pool
GetUniverse = lambda x, y: x.columns[x.loc[y]].tolist()
trade_dt = [x for x in all_tradedates if (x >= from_date) & (x < to_date)]
...
for date in factor_dt:
    ...
    t_3 = time.time()
    print(date, int(tag[tag['TAG'] < 0].abs().sum()), int(tag[tag['TAG'] > 0].sum()), t_3-t_2, t_3-t_1)
    if use_short:
